...
!(function ($, options) {
    ...
    function fireEvent(name, allData) {
      ...
      if (options.facebook.serverApiEnabled) {
        ...
        if (
          ...
          options.facebook.ajaxForServerEvent ||
          isApiDisabled ||
          allData.delay > 0 ||
          allData.type !== "static"
        ) {
  ...
  $(document).ready(function () {
    ...
    if (options.dynamicEvents.hasOwnProperty("signal_page_scroll")) {
      var singlePageScroll = function () {
        ...
        for (var i = 0; i < pixels.length; i++) {
          var event = Utils.clone(
            options.dynamicEvents.signal_page_scroll[pixels[i]]
          );
    ...
    if (options.dynamicEvents.hasOwnProperty("signal_time_on_page")) {
      ...
      var time =
        options.dynamicEvents.signal_time_on_page[pixels[0]].time_on_page; // the same for all pixel
      setTimeout(function () {
        for (var i = 0; i < pixels.length; i++) {
          var event = Utils.clone(
            options.dynamicEvents.signal_time_on_page[pixels[i]]
          );
          Utils.copyProperties(Utils.getRequestParams(), event.params);
          ...
          if (options.dynamicEvents.hasOwnProperty("signal_download")) {
            ...
            if ($elem.is("a")) {
              ...
              if (extension.length > 0) {
                ...
                for (var i = 0; i < pixels.length; i++) {
                  var event = Utils.clone(
                    options.dynamicEvents.signal_download[pixels[i]]
                  );
    ...
    if (options.dynamicEvents.hasOwnProperty("signal_comment")) {
      $("form.comment-form").on("submit", function () {
        ...
        for (var i = 0; i < pixels.length; i++) {
          var event = Utils.clone(
            options.dynamicEvents.signal_comment[pixels[i]]
          );
          Utils.copyProperties(Utils.getRequestParams(), event.params);
